...
if not image_items:
    # or handle the error, e.g., raise Exception("No image items found!")
    image_data = []
...
def select_sample_text(target_lang_code):
    ...
    # Par défaut si langue non trouvée
...
async def simulate_validation(message: Message, state: FSMContext):
    ...
    if validation_result:
        ...
        image_ready_kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(
                    text="🖼️ Start Image Test", callback_data="start_image_test")]
            ]
        )
        ...
        logger.info(f"✅ TRANSLATION PASSED - User {message.from_user.id}")
        logger.info(f"Translation: {data.get('user_translation')}")
...
async def handle_next_step(message: Message, state: FSMContext):
    ...
    if num_q < 2:
        ...
        video_ready_kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(
                    text="🖼️ Start Video Test", callback_data="start_video_test")]
            ]
        )
...
async def handle_next_video_step(message: Message, state: FSMContext):
    ...
    if num_q < 2:
        ...
        start_tasks_kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(
                    text="Start Real Tasks", callback_data="start_real_tasks")],
                [InlineKeyboardButton(
...
async def handle_view_commands(callback: CallbackQuery, state: FSMContext):
    ...
    start_kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Go to Tasks",
                                  callback_data="start_real_tasks")]
        ]
    )
