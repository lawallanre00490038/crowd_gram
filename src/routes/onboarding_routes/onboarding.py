...
async def cmd_start(message: Message, state: FSMContext):
    logger.info(
        f"🔍 [DEBUG] /start command received from user {message.from_user.id}")
...
async def get_state(message: Message, state: FSMContext):
    ...
    if len(states) > 0:
        ...
        await message.answer(
            LOCATION_MSG['select_state'].format(
...
async def get_referrer(message: Message, state: FSMContext):
    ...
    if result["success"]:
        ...
        logger.trace(message.from_user.id,
                     "has completed onboarding with data:", user_data)
