...
from .prompt import CONTEST_PROMPT, POLL_PROMPT, TRIVIA_PROMT, WELLNESS_PROMPT
...
async def send_monthly_contest():
    ...
    while True:
        try:
            ...
            logger.trace(f"Error sending contest message: {e}")
...
async def broadcast_new_trainings():
    ...
    try:
        ...
        logger.error(f"Community-Broadcast Policies Router Error: Details: {err}")  # noqa: E501
...
async def generate_poll(message_text: str) -> tuple[str, list[str]]:
    ...
    try:
        ...
        logger.info(f"LLM generated options: {options}")
...
async def daily_trivia():
    ...
    while True:
        try:
            ...
            logger.exception("Trivia session failed.")
