...
def check_video_file_length(file_path: str, min_length: float = 0.5, max_length: float = 600.0) -> bool:
    ...
    try:
        ...
        if not cap.isOpened():
            logger.info(f"Error: Unable to open video file: {file_path}")
            return False
        ...
        if fps == 0:
            logger.info("Error: FPS is zero, can't calculate duration.")
            return False
        ...
        logger.info(f"Error checking video length: {e}")
        return False
...
def check_video_frame_rate(file_path: str, expected_frame_rate: float = 30.0) -> bool:
    ...
    try:
        ...
        logger.info(f"Error checking video frame rate: {e}")
        return False
...
def check_video_bit_depth(file_path: str, expected_bit_depth: int = 8) -> bool:
    ...
    try:
        ...
        if not ret:
            logger.info("Error: Couldn't read a frame.")
            return False
        ...
        if frame.dtype == 'uint8':
            ...
            logger.info(f"Unrecognized dtype: {frame.dtype}")
            return False
    except Exception as e:
        logger.info(f"Error checking video bit depth: {e}")
        return False
