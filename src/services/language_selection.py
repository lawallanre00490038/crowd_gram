...
async def handle_language_keyboard_callback(data: str, state: FSMContext):
    ...
    if action == "page":
        ...
        dynamic_kb = load_languages_kb(languages, page_num, show_done=show_done)
        return "", dynamic_kb
    ...
    if action == "select":
        ...
        dynamic_kb = None if status else load_languages_kb(languages, page_num, show_done=show_done)
