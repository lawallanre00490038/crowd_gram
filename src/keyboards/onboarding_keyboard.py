    ...
    for variable in signuplist.data.result:
        if variable.id == category:
            for field_data in variable.field_data:
                keyboard.append(InlineKeyboardButton(text = field_data.name, callback_data= f"{field_data.field_id}:{field_data.name}"))
...
def load_inline_keyboard(items: List[str], page: int = 0, show_done = False) -> InlineKeyboardMarkup:
    ...
    print(start, end, ITEMS_PER_PAGE, len(items))

    for index, item in enumerate(items[start:end]):
        markup.append(InlineKeyboardButton(text=item, callback_data=f"select:{index}:{page}"))
    ...
    if show_done:
        nav_buttons.append(InlineKeyboardButton(text="✅ Done", callback_data=f"✅ Done"))
    ...
    if len(items) > ITEMS_PER_PAGE:
        ...
        if end < len(ITEMS):
            nav_buttons.append(InlineKeyboardButton(text="Next ➡️", callback_data=f"page:{page + 1}"))
...
async def task_type_kb(categories: List[Category], show_done = False):
    ...
    for index, cat in enumerate(categories):
        markup.append([InlineKeyboardButton(text=cat.name, callback_data=f"select:{index}")])

    if show_done:
        markup.append([InlineKeyboardButton(text="✅ Done", callback_data=f"done:")])
